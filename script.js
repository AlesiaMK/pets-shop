...
function makeCardByTemplate(itemsShop) {
    const { title, description, img, price, tags } = itemsShop;
...
items.forEach((el) => {
    const itemsShop = el;
    const myCard = makeCardByTemplate(itemsShop);
...
function searchProduct() {
    ...
    const result = items.filter((item) => item.title.trim().toLocaleLowerCase().includes(searchInput))
    ...
    if (!searchInput.length) {
        ...
        result.forEach((item) => {
            shopItems.append(makeCardByTemplate(item))
        })
    };
